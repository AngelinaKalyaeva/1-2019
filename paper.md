...
В качестве API для взаимодействия с интернет магазинами и проверки гипотезы о работоспособности предлогаемого выше решения(MVP(мимнимальный значимый продукт) POF(доказательство концепции)) был развернут локально веб-сервис написанный на языке java с использованием технологий spring boot и graphql api, который использует типизированную схему данных, которую можно представить в json-подобном формате. Так как предлгаемое решение взаимодействует как с метриками работоспособности, так и аналитическими метриками, то внутри схемы данных было вынесено два независимых подмножества элементов. В первую очередь был рассмотрен ряд метрик работоспособности приложения, который может и будет расширяться в дальнейшем:
  * Ошибки платежной системы - так как взаимодействие с платежной системой происходит по https, то сохраняем коды ответов сервера платежной системы, а в качестве метрики рассчитывается соотношение успешных ответов сервера к ошибкам за определенный интревал времени заданный клиентов. А так же смотрим на количество зависших платежей, которые находятся в очереди.
  * Ошибки по обращению к url - взаиможествие с интрет-магазинами осуществляется по определенным протоколам, зачаствую это http(могут быть и другие но значительно реже), таким образом для каждого url сохраняем успешность ответа и рассчитываем процент успешных обращений за определенных промежуток времени заданный пользователем. 
  * Тайминги на обращение к url - для каждого url пользователем задается соглашение сервисного уровня, которое описывает описывает время ответа сервера, за которое тот обязуется выполнить запрос. Таким образом для каждого url сохраняем результат успешности выполнения соглашения. Далее рассчитываем соотношение успешных ответов к неуспехным за определенный промежуток времени заданным пользователем. 
  * Снижение количества запросов в секунду -  Для каждого url, который предоставляет интернет магазин рассчитывается количество запросов с секунду на данный момент. В качестве метрики считается  процент запросов насколько снизилось это количество по сравнению со вчерашним днем 
  * Ошибки при обращении к базе данных - успешность выполнения запросов к базе данных интернет магазина
  * Тайминги на работу с базой данных - время ответа запросов при обращении к базе данных.
  * Метрики на ресурсы сервиса - нагрузка на процессор, оперативная память, нагрузка на сеть, заполненность жестких дисков.
  
В качестве аналитических метрик были выбраны:
  * Посещаемость магазина 
k=Nt  - коэффициент посещаемости, где N – количество обращений к сайту за время t
alert_ifkavgkН<0.7
  * Динамика продаж 
k=Nt  - коэффициент продаж, где N – количество продаж за время t
alert_ifkavgkН<0.7
